...
<%if(session.getAttribute("datosUsuario")!=null && Integer.parseInt(String.valueOf(((Object[])session.getAttribute("datosUsuario"))[5]))==3){ %>
	...
	<head>
		...
		<script type="text/javascript" src="js/dataTables-bs4.min.js"></script>
	...
	<body>
		...
		<div id="contenedor">
			...
			<!-- CUERPO -->
			<div class="row">
				<div class="col-xs-12 col-md-10 push-md-1" id="cuerpoAdmin">
					<h1 id="tituloAdmin">Gestión de usuarios</h1>
					<div class="row">
						<div class="col-xs-12" id="panelUsuarios">
							<table class="table table-responsive table-striped tablaMantenimiento">
								<thead class="thead-inverse">
									...
									<th>Incidencias</th>
									<th>Modificar</th>
									<th>Banear/Desbanear usuario</th>
									<th>Añadir incidencia</th>
								</thead>
								<tbody>
									<!-- CARGADA DINAMICAMENTE -->
								</tbody>
							</table>
						</div>
					</div>
		...
		<%
			/*MENSAJES TOASTR, para cuando se actualiza con exito un usuario*/
			if(request.getSession().getAttribute("exitoModificacion")!=null){
		%>
			<script type="text/javascript">
				toastr.success('<%=String.valueOf(request.getSession().getAttribute("exitoModificacion"))%>');
			</script>
		<%
			request.getSession().removeAttribute("exitoModificacion");
			}
