...
<body>
	...
	<div id="contenedor">
		<div class="col-xs-12" id="background-blanco">
			<div class="col-xs-12" id="carga">
				<img src="img/loading.png" class="img-fluid">
			</div>
		</div>
		<!--CABECERA-->
		...
		<!-- CUERPO -->
		<div class="row">
			<div class="col-xs-12 col-md-11" id="cuerpoSubforo">
				<%
					if(String.valueOf(session.getAttribute("estado")).equals("Conectado")){
				%>
					<a href='nuevoTema.jsp?id=<%=request.getParameter("id")%>' class="btn btn-warning" id="nuevoTema">Nuevo tema</a>
				<%
					}
				%>
				<input type="hidden" id="pagina" value='<%=request.getParameter("pag")%>'>
				<input type="hidden" id="subforo" value='<%=request.getParameter("id")%>'>		
				<div id="temas">
	...
	<%
		/*MENSAJES TOASTR, para cuando se crea con exito un nuevo tema*/
		if(request.getSession().getAttribute("exitoNuevoTema")!=null){
	%>
		<script type="text/javascript">
			toastr.success('<%=String.valueOf(request.getSession().getAttribute("exitoNuevoTema"))%>');
		</script>
	<%
		request.getSession().removeAttribute("exitoNuevoTema");
		}
	...
	<%
		/*MENSAJES TOASTR, para cuando no se crea con exito un nuevo tema*/
		if(request.getSession().getAttribute("errorNuevoTema")!=null){
	%>
		<script type="text/javascript">
			toastr.error('<%=String.valueOf(request.getSession().getAttribute("errorNuevoTema"))%>');
		</script>
	<%
		request.getSession().removeAttribute("errorNuevoTema");
		}
